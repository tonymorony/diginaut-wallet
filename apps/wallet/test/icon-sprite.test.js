...
function definedSymbols() {
  return new Set([...html.matchAll(/<symbol\s+id="(ic-[a-z0-9-]+)"/g)].map((m) => m[1]));
}
...
test('no symbol carries a presentation attribute', () => {
  ...
  const offenders = [];
  for (const m of html.matchAll(/<symbol\s+id="(ic-[a-z0-9-]+)"[^>]*>([\s\S]*?)<\/symbol>/g)) {
    const [, id, body] = m;
    for (const attr of ['stroke-width', 'stroke', 'fill']) {
      if (new RegExp(`\\s${attr}="`).test(body)) offenders.push(`${id} → ${attr}`);
    }
  }
  assert.deepEqual(offenders, [], 'presentation attributes defeat the size-class ladder');
});

test('every symbol in the sprite is rendered somewhere', () => {
  // Not a correctness failure — an unused symbol is dead weight, not a bug — but
  // the count is worth pinning so the set does not quietly grow a tail nobody
  // draws. Update the list deliberately when adding a symbol ahead of its use.
  ...
  assert.deepEqual(
    ...
    'the unused-symbol inventory moved — render it, drop it, or update this list',
  );
});
