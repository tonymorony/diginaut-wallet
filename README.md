...
**Try it: <https://diginaut.space>** — create a wallet in seconds and explore the full
dashboard, mint calculator, and send/receive flows. A permanent **testnet instance at
<https://testnet.diginaut.space>** lets you exercise the complete mint / transfer / redeem cycle
with valueless coins: create a wallet, claim testnet DGB from the built-in faucet, and mint
DigiDollar end-to-end. The original addresses — <https://diginaut.ludere.space> and
<https://dgb.ludere.space> — keep serving the same instances and are never redirected, because a
browser wallet lives per site and a redirect would cut existing users off from theirs. Both run
the full stack in this repo against a DigiByte Core **v9.26.4** node with a real oracle price
feed. Want to host your own? See [docs/SELF-HOSTING.md](docs/SELF-HOSTING.md).
